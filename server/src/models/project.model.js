...
export async function createProject({ title, abstract, authorName, departmentId, uploadedBy, embedding }) {
  const result = await pool.query(
    `INSERT INTO projects (title, abstract, author_name, department_id, uploaded_by, embedding)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
    [title, abstract, authorName, departmentId, uploadedBy, embedding]
  );
