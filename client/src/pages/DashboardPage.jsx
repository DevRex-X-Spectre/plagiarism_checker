    ...
    <div className="py-8 lg:py-12">
      <div className="container max-w-5xl">
        <div className="mb-10 animate-fade-up">
          ...
          <h1 className="text-3xl lg:text-4xl font-light text-deep-ink tracking-tight">
            Welcome, {firstName}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
          ...
          <ActionCard icon={Library} title="Browse archive" href="/browse" delay={300} />
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="animate-fade-up delay-200">
            <h2 className="text-lg font-medium text-deep-ink mb-4 flex items-center gap-2">
              <FileText className="w-5 h-5 text-slate" /> My uploads ({myProjects.length})
            </h2>
            {loading ? (
              <Card><p className="text-center py-8 text-slate">Loading...</p></Card>
            ) : myProjects.length === 0 ? (
              <Card>
                <EmptyState icon="upload" title="No projects yet" description="Upload your first project." actionLabel="Upload" actionIcon={Upload} action={() => window.location.href = '/upload'} />
              </Card>
            ) : (
                    ...
                    <Card padding={16} hover>
                      <div className="flex items-center justify-between">
                        <div className="min-w-0 flex-1 mr-4">
                          <p className="text-sm font-medium text-deep-ink line-clamp-2">{p.title}</p>
          ...
          <div className="animate-fade-up delay-300">
            ...
            {loading ? (
              ...
              <Card>
                <EmptyState icon="search" title="No checks yet" description="Run your first similarity check." actionLabel="Check now" actionIcon={Search} action={() => window.location.href = '/similarity-check'} />
              </Card>
            ) : (
              <div className="space-y-3">
                {checkHistory.slice(0, 5).map((c, i) => (
                  <Link key={c.id} to={`/similarity/${c.id}`} className="block animate-fade-up" style={{ animationDelay: `${i * 50}ms` }}>
                    <Card padding={16} hover>
                      <p className="text-sm font-medium text-deep-ink line-clamp-2 mb-2">"{c.query_text}"</p>
                      <div className="flex items-center justify-between">
...
function ActionCard({ icon: Icon, title, href, delay }) {
  return (
    <Link to={href} className="block animate-fade-up" style={{ animationDelay: `${delay}ms` }}>
      <Card padding={20} hover className="group">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-lg bg-pale-cyan/30 flex items-center justify-center">
            <Icon className="w-5 h-5 text-forest-teal" />
          </div>
          <div className="flex-1">
            <h3 className="text-base font-medium text-deep-ink">{title}</h3>
          </div>
