        ...
        {step === 'upload' && (
          <Card padding={32} className="card-3d">
            ...
            <div onClick={() => fileRef.current.click()} onDrop={handleDrop} onDragOver={e => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)}
              className={`border-2 border-dashed rounded-[24px] p-12 text-center cursor-pointer transition-all ${dragActive ? 'border-deep-indigo bg-pale-cyan/10 shadow-[0_18px_36px_-26px_rgba(17,26,74,0.4)]' : 'border-mist hover:border-deep-indigo hover:bg-white/60'}`}>
              {loading ? (
        ...
        {step === 'review' && (
          <Card padding={24} className="card-3d">
            ...
            <div className="space-y-4">
              ...
              <div><label className="block text-sm font-medium mb-2 text-carbon">Abstract *</label><textarea value={form.abstract} onChange={e => setForm({ ...form, abstract: e.target.value })} rows={5} className="w-full px-4 py-3 border border-mist rounded-2xl bg-card-white outline-none transition-all duration-150 focus:border-deep-indigo focus:shadow-[0_0_0_4px_rgba(17,26,74,0.12)] resize-vertical" /></div>
...
function Step({ n, label, active, done }) {
  return (
    <div className="flex items-center gap-2">
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-all ${done ? 'bg-success text-white shadow-[0_12px_24px_-16px_rgba(5,150,105,0.45)]' : active ? 'bg-deep-indigo text-white scale-110 shadow-[0_14px_26px_-16px_rgba(17,26,74,0.55)]' : 'bg-mist text-slate'}`}>
